...
class GalaxyData {
    ...
    async deployUpgradeable(adminAddress: any, paramAdd: any,
    ) {
        ...
        const contract = await ethers.getContractFactory("GalaxyData");
        console.log("GalaxyData.deploying ...")
        const proxy = await upgrades.deployProxy(contract, [adminAddress, paramAdd], {
            initializer: 'initialize(address, address)',
        });
        await proxy.deployed();
        console.log("GalaxyData deployed at proxy:", proxy.address);
        return proxy.address;
    }

    getContract(contractAddress: any, contractName: any = "./artifacts/contracts/data/GalaxyData.sol/GalaxyData.json") {
    ...
    async tokenHTML(contractAddress: any, seed: any) {
        ...
        const val: any = await temp?.nftContract.methods.tokenHTML(seed).call(tx);
        return val;
    }

    async tokenURI(contractAddress: any, seed: any) {
        let temp = this.getContract(contractAddress);
        const nonce = await temp?.web3.eth.getTransactionCount(this.senderPublicKey, "latest") //get latest nonce
        ...
        const val: any = await temp?.nftContract.methods.tokenURI(seed).call(tx);
        return val;
    }
}
